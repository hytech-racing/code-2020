...
void MCU_status::load(uint8_t buf[8]) {
    ...
    memcpy(&(message.flags), &buf[1], sizeof(uint16_t));
    memcpy(&(message.temperature), &buf[3], sizeof(int16_t));
    memcpy(&(message.glv_battery_voltage), &buf[5], sizeof(uint16_t));
}
...
void MCU_status::write(uint8_t buf[8]) {
    ...
    memcpy(&buf[1], &(message.flags), sizeof(uint16_t));
    memcpy(&buf[3], &(message.temperature), sizeof(int16_t));
    memcpy(&buf[5], &(message.glv_battery_voltage), sizeof(uint16_t));
}
...
bool MCU_status::get_bspd_ok_high() {
    return (message.flags & 0x90) >> 8;
}
...
void MCU_status::set_bms_ok_high(bool bms_ok_high) {
    message.flags = (message.flags & 0xFFFE) | (bms_ok_high & 0x1);
}

void MCU_status::set_imd_okhs_high(bool imd_okhs_high) {
    message.flags = (message.flags & 0xFFFD) | ((imd_okhs_high & 0x1) << 1);
}

void MCU_status::set_inverter_powered(bool inverter_powered) {
    message.flags = (message.flags & 0xFFFB) | ((inverter_powered & 0x1) << 2);
}

void MCU_status::set_shutdown_b_above_threshold(bool shutdown_b_above_threshold) {
    message.flags = (message.flags & 0xFFF7) | ((shutdown_b_above_threshold & 0x1) << 3);
}

void MCU_status::set_shutdown_c_above_threshold(bool shutdown_c_above_threshold) {
    message.flags = (message.flags & 0xFFEF) | ((shutdown_c_above_threshold & 0x1) << 4);
}

void MCU_status::set_shutdown_d_above_threshold(bool shutdown_d_above_threshold) {
    message.flags = (message.flags & 0xFFDF) | ((shutdown_d_above_threshold & 0x1) << 5);
}

void MCU_status::set_shutdown_e_above_threshold(bool shutdown_e_above_threshold) {
    message.flags = (message.flags & 0xFFBF) | ((shutdown_e_above_threshold & 0x1) << 6);
}

void MCU_status::set_shutdown_f_above_threshold(bool shutdown_f_above_threshold) {
    message.flags = (message.flags & 0xFF7F) | ((shutdown_f_above_threshold & 0x1) << 7);
}

void MCU_status::set_bspd_ok_high(bool bspd_ok_high) {
    message.flags = (message.flags & 0xFEFF) | ((bspd_ok_high & 0x1) << 8);
}
