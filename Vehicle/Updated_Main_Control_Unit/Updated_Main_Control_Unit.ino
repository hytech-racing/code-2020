...
#define SHUTDOWN_D_READ A4
#define BMS_OK_READ A5
#define OK_HS_READ A6
#define BSPD_OK_READ A7
#define FAN_1 A8
#define FAN_2 A9
...
#define BMS_HIGH 465                    // ~3V on BMS_OK line
#define IMD_HIGH 465                    // ~3V on OKHS line
#define BSPD_HIGH 465                   // ~3V on BSPD line
#define SHUTDOWN_B_HIGH 530             // ~5V on SHUTDOWN_B line
#define SHUTDOWN_C_HIGH 350             // ~5V on SHUTDOWN_C line
#define SHUTDOWN_D_HIGH 465             // ~5V on SHUTDOWN_D line ??????
...
void read_status_values() {
  ...
  */
  ...
  /*
     Check for BMS fault
  */
  if (analogRead(BMS_OK_READ) > BMS_HIGH) {
    mcu_status.set_bms_ok_high(true);
  } else {
  ...
  /*
     Check for IMD fault
  */
  if (analogRead(OK_HS_READ) > IMD_HIGH) {
    mcu_status.set_imd_okhs_high(true);
  } else {
  ...
  */
  if (analogRead(BSPD_OK_READ) > BSPD_HIGH) {
    mcu_status.set_bspd_ok_high(true);
  } else {
    mcu_status.set_bspd_ok_high(false);
    if (timer_imd_print_fault.check()) {
      Serial.println("BSPD fault detected");
    }
  }
  ...
  */
  ...
  mcu_status.set_shutdown_d_above_threshold(analogRead(SHUTDOWN_D_READ) > SHUTDOWN_D_HIGH);
